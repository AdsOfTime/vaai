...
function App() {
  ...
  useEffect(() => {
    // Check for OAuth code in URL (direct callback from Google)
    ...
    if (storedToken) {
      try {
        const userData = JSON.parse(atob(storedToken.split('.')[1])); // Decode JWT payload
  ...
  const handleGoogleLogin = async () => {
    try {
      // Get the OAuth URL from the Worker
  ...
  const loadData = async () => {
    try {
      ...
      const [emailRes, calendarRes, tasksRes] = await Promise.all([
        ...
        fetch(`${API_BASE_URL}/api/calendar`, { headers }).catch(() => null),
  ...
  const sendEmail = async () => {
    ...
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(`${API_BASE_URL}/api/gmail/compose/send`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          ...(token ? { 'Authorization': `Bearer ${token}` } : {})
  ...
  const createDocument = async () => {
    if (!docTitle.trim() || !docContent.trim() || docCreating) return;
    
    setDocCreating(true);
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(`${API_BASE_URL}/api/google/docs`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          ...(token ? { 'Authorization': `Bearer ${token}` } : {})
      ...
      if (response.ok) {
        ...
        setDocTitle('');
        setDocContent('');
        alert(`Document created successfully! Document ID: ${data.document.documentId}`);
      } else {
        const errorData = await response.json();
        alert(`Failed to create document: ${errorData.message || 'Unknown error'}`);
      }
    } catch (error) {
    ...
    <div style={{ 
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <main style={{ flex: 1, padding: '24px', overflow: 'auto' }}>
            ...
            <div style={{ maxWidth: '800px', margin: '0 auto' }}>
              <div style={{
                ...
                <button
                  ...
                  style={{
                    background: emailSending ? '#9ca3af' : '#3b82f6',
                    color: 'white',
                    border: 'none',
                    padding: '12px 24px',
                    borderRadius: '8px',
          ...
          {/* Data Analysis - Google Docs Creation */}
          {currentView === 'analyzer' && (
            <div style={{ maxWidth: '800px', margin: '0 auto' }}>
              <div style={{
                ...
                <h3 style={{ fontSize: '20px', fontWeight: '600', color: '#1e293b', marginBottom: '24px' }}>
                  📈 Data Analysis - Create Google Doc
                </h3>
                ...
                <div style={{ marginBottom: '24px' }}>
                  ...
                  <textarea
                    ...
                    placeholder="Enter your analysis content, data insights, or report details..."
                    rows={10}
                    style={{
                      width: '100%',
                      padding: '12px',
                ...
                <button
                  ...
                  disabled={docCreating || !docTitle.trim() || !docContent.trim()}
                  style={{
                    background: docCreating ? '#9ca3af' : '#059669',
                    color: 'white',
                    border: 'none',
                    padding: '12px 24px',
                    borderRadius: '8px',
                    fontSize: '14px',
                    fontWeight: '500',
                    cursor: docCreating ? 'not-allowed' : 'pointer'
                  }}
                >
                  {docCreating ? 'Creating...' : 'Create Google Doc'}
                </button>
              </div>
            </div>
          )}

          {/* Other views that still need implementation */}
          {(currentView === 'focus' || currentView === 'calendar' || currentView === 'tasks') && (
