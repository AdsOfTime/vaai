...
function App() {
  ...
  useEffect(() => {
    // Immediately set demo user and stop loading for testing
    console.log('Starting VAAI in demo mode for debugging');
  ...
  const checkAuthStatus = async () => {
    try {
      ...
      if (response.ok && response.headers.get('content-type')?.includes('application/json')) {
        ...
        if (userData && !userData.error) {
          setUser(userData);
          setLoading(false);
          return; // Successfully got real user data
        }
      }
    } catch (error) {
    ...
    // If no authenticated user found, show login instead of demo
  ...
  const loadData = async () => {
    try {
      ...
      // Only use mock data if no real APIs worked
      if (!hasRealData) {
        ...
        setEmails([
          ...
          { id: 2, subject: 'Project Deadline Extension', from: 'mike@client.com', category: 'priority', time: '4 hours ago' },
          { id: 3, subject: 'Team Standup Notes', from: 'team@company.com', category: 'work', time: '1 hour ago' },
          { id: 4, subject: 'Weekend Plans', from: 'friend@gmail.com', category: 'personal', time: '3 hours ago' }
        ]);
